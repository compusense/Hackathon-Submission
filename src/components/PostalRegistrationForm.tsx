...
export default function PostalRegistrationForm() {
  ...
  const processNext = async () => {
    ...
    if (currentStep === 1) fieldsToValidate = ['serviceType', 'serviceDescription', 'geographicalCoverage'];
  ...
  const onSubmit = (data: RegistrationFormData) => {
    console.log('Postal Operator Registration submitted:', { ...data, files });
  ...
  if (isSubmitted) {
    return (
      ...
      >
        <div className="w-24 h-24 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center">
          <CheckCircle2 className="w-12 h-12" />
        </div>
        <h2 className="text-4xl font-display font-bold text-sector-postal dark:text-red-400">Registration Submitted</h2>
        <p className="text-gray-600 dark:text-gray-400 max-w-lg text-lg">
          Your postal operator registration for <strong>{watch('applicantName')}</strong> has been successfully received by BOCRA and paid.
        </p>
        <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-2xl w-full max-w-md my-4">
          ...
          <p className="text-3xl font-mono font-bold text-sector-postal">REG-POST-{new Date().getFullYear()}-{Math.floor(Math.random() * 9000) + 1000}</p>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-xl overflow-hidden">
      <div className="bg-sector-postal p-8 pb-20 text-white">
        <h2 className="text-2xl font-display font-bold mb-12 text-center">Postal Operator Registration</h2>
          ...
          <div className="flex justify-between items-center relative">
            ...
            {steps.map((step, i) => (
              <div key={step.id} className="relative z-10 flex flex-col items-center">
                <div className={cn(
                  "w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 border-4 border-sector-postal",
                  i <= currentStep ? "bg-white text-sector-postal scale-110" : "bg-white/50 text-white/50"
                )}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8">
          <AnimatePresence mode="wait">
            {currentStep === 0 && (
              ...
              >
                <div className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <input {...register('applicantName')} className="input-field" placeholder="e.g. Gaborone Courier Services Ltd" />
            ...
            {currentStep === 1 && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-bold text-gray-600 dark:text-gray-300">Type of Postal Service</label>
                  ...
                  <div className="flex flex-col gap-2 md:col-span-2">
                    ...
                    <textarea {...register('serviceDescription')} rows={4} className="input-field resize-none" placeholder="Describe the nature of postal/courier services to be provided..." />
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex gap-4">
                    <select className="input-field w-1/3" onChange={(e) => {
                      const fileInput = document.getElementById('fileInputPostal') as HTMLInputElement;
                  ...
                  <div 
                    ...
                    className={cn(
                      ...
                      isDragging 
                        ? "border-sector-postal bg-red-50 dark:bg-red-900/10" 
                        : "border-gray-300 dark:border-gray-700 hover:border-sector-postal dark:hover:border-red-500 bg-gray-50 dark:bg-gray-800/50"
                    )}
                  >
                    <UploadCloud className="w-10 h-10 text-sector-postal mb-4" />
                    ...
                    <label className="bg-sector-postal text-white px-6 py-3 rounded-xl font-bold cursor-pointer hover:bg-red-700 transition-all">
                      Select Files
                      <input id="fileInputPostal" type="file" multiple className="hidden" onChange={handleFileInput} />
                    </label>
                  </div>
                      ...
                      {files.map((file, i) => (
                        <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl">
                          <div className="flex items-center gap-3 overflow-hidden">
                            <File className="w-5 h-5 text-sector-postal shrink-0" />
            ...
            {currentStep === 3 && (
              ...
              >
                <div className="bg-gray-50 dark:bg-gray-800/50 p-8 rounded-3xl border border-gray-200 dark:border-gray-700">
                  <h3 className="text-xl font-bold text-sector-postal dark:text-red-400 mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">Registration Summary</h3>
                ...
                <div className="flex items-start gap-4 p-6 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/50 rounded-2xl">
                  <input type="checkbox" {...register('declaration')} id="declaration" className="mt-1 w-5 h-5 accent-sector-postal shrink-0" />
                  <div className="flex flex-col gap-2">
            ...
            {currentStep === 4 && (
              ...
              >
                <PaymentStep 
                  referenceNumber={`REG-POST-${new Date().getFullYear()}-${Math.floor(Math.random() * 9000) + 1000}`}
          ...
          <div className="flex justify-between items-center pt-8 border-t border-gray-100 dark:border-gray-800 mt-4">
            <button
              ...
              className={cn(
                ...
                (currentStep === 0 || currentStep === 4) ? "text-gray-300 dark:text-gray-700 cursor-not-allowed" : "text-sector-postal hover:bg-red-50 dark:hover:bg-gray-800"
              )}
            >
            ...
            {currentStep < steps.length - 1 ? (
              <button
                type="button"
                onClick={processNext}
                className="bg-sector-postal text-white hover:bg-red-700 px-8 py-3 rounded-xl font-bold transition-all flex items-center gap-2"
              >
