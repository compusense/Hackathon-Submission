...
const registrationSchema = z.object({
  ...
  spectrumRequired: z.boolean(),
  spectrumDetails: z.string().optional(),
...
export default function TelecommunicationRegistrationForm() {
  ...
  const {
    ...
    mode: 'onChange',
    defaultValues: {
      spectrumRequired: false,
    }
  });

  const spectrumRequired = watch('spectrumRequired');
  ...
  const onSubmit = (data: RegistrationFormData) => {
    console.log('Telecommunication Registration submitted:', { ...data, files });
  ...
  if (isSubmitted) {
    return (
      ...
      >
        ...
        <h2 className="text-4xl font-display font-bold text-bocra-blue dark:text-blue-400">Registration Submitted</h2>
        <p className="text-gray-600 dark:text-gray-400 max-w-lg text-lg">
          Your telecommunication registration for <strong>{watch('applicantName')}</strong> has been successfully received by BOCRA and paid.
        </p>
        <div className="bg-bocra-light dark:bg-gray-800 p-6 rounded-2xl w-full max-w-md my-4">
          ...
          <p className="text-3xl font-mono font-bold text-bocra-blue">REG-TEL-{new Date().getFullYear()}-{Math.floor(Math.random() * 9000) + 1000}</p>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-xl overflow-hidden">
      ...
      <div className="bg-bocra-blue dark:bg-blue-900 p-8 pb-20 text-white">
        <h2 className="text-2xl font-display font-bold mb-12 text-center">Telecommunication Registration</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8">
          <AnimatePresence mode="wait">
            ...
            {currentStep === 1 && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-bold text-gray-600 dark:text-gray-300">Type of Service</label>
                    <select {...register('serviceType')} className="input-field">
                      <option value="">Select service type...</option>
                      <option value="isp">Internet Service Provider (ISP)</option>
                      <option value="vas">Value Added Services (VAS)</option>
                      <option value="pnl">Private Network License</option>
                      <option value="data">Data Center Services</option>
                      <option value="voip">Voice over IP (VoIP)</option>
                    </select>
                  ...
                  <div className="flex flex-col gap-2 md:col-span-2">
                    ...
                    <textarea {...register('serviceDescription')} rows={4} className="input-field resize-none" placeholder="Describe the nature of telecommunication services to be provided..." />
                  ...
                  <div className="flex flex-col gap-2 md:col-span-2">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {['Fiber Optic', 'Wireless/Radio', 'Satellite/VSAT', 'Copper/Leased'].map(type => (
                        <label key={type} className="flex items-center gap-2 p-3 border border-gray-200 dark:border-gray-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                          <input type="radio" value={type} {...register('infrastructureType')} className="accent-bocra-blue" />
                  ...
                  <div className="md:col-span-2 p-4 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-100 dark:border-yellow-900/30 rounded-2xl flex items-start gap-3">
                    <Info className="w-5 h-5 text-yellow-600 shrink-0 mt-0.5" />
                    <div className="flex flex-col gap-3 w-full">
                      <div className="flex items-center justify-between">
                        <label className="text-sm font-bold text-yellow-800 dark:text-yellow-400">Is Radio Spectrum Required?</label>
                        <input type="checkbox" {...register('spectrumRequired')} className="w-5 h-5 accent-yellow-600" />
                      </div>
                      {spectrumRequired && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                          <textarea {...register('spectrumDetails')} rows={2} className="w-full p-3 rounded-xl border border-yellow-200 dark:border-yellow-800 bg-white dark:bg-gray-900 text-sm" placeholder="Specify frequency bands and bandwidth required..." />
                        </motion.div>
                      )}
                    </div>
                  </div>
            ...
            {currentStep === 2 && (
              ...
              >
                ...
                <div className="flex flex-col gap-4">
                  ...
                  >
                    <UploadCloud className="w-10 h-10 text-bocra-blue mb-4" />
                    <h3 className="text-lg font-bold text-gray-700 dark:text-gray-200 mb-1">Upload Business Plan</h3>
                      ...
                      {files.map((file, i) => (
                        <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl">
                          <div className="flex items-center gap-3 overflow-hidden">
                            <File className="w-5 h-5 text-bocra-blue shrink-0" />
            ...
            {currentStep === 3 && (
              ...
              >
                <div className="bg-gray-50 dark:bg-gray-800/50 p-8 rounded-3xl border border-gray-200 dark:border-gray-700">
                  <h3 className="text-xl font-bold text-bocra-blue dark:text-blue-400 mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">Registration Summary</h3>
            ...
            {currentStep === 4 && (
              ...
              >
                <PaymentStep 
                  referenceNumber={`REG-TEL-${new Date().getFullYear()}-${Math.floor(Math.random() * 9000) + 1000}`}
